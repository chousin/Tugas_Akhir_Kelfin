    ...
    <div class="card-body">
        ...
        <form method="POST" action="{{ url('/pengajuan-penggajian') }}">
            ...
            <div class="mb-3">
                ...
                <input type="date" name="periode_start" class="form-control" value="{{ date('Y-m-d') }}" min="{{ isset($pengajuan_penggajian) ? date('Y-m-d', strtotime('+1 days', strtotime($pengajuan_penggajian->periode_end))) : ''; }}">
            </div>
            <div class="mb-3">
                <label for="periode_end" class="form-label">Periode Absen Akhir</label>
                <input type="date" name="periode_end" class="form-control" value="{{ date('Y-m-d') }}" min="{{ isset($pengajuan_penggajian) ? date('Y-m-d', strtotime('+1 days', strtotime($pengajuan_penggajian->periode_end))) : ''; }}">
            </div>
            <div class="mb-3">
